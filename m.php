      ...
      $currentrater = $raters[$rated];
       ...
       foreach ($acct2rate as $beingrated) : 
           ...
           $verifiedaccount = $jsonm["data"]["0"]["userName"];
               ...
               print "$White\n";
               printf("%-6s (%2s/%2s) | %-15s\n","Raters",$rated,$noofraters,$currentrater);
               printf("%-6s (%2s/%2s) | %-15s\n","Main",$donerating,$toratecount,$beingrated);
               ...
               $arr = json_decode($respd, true);
   		       ...
   		       $devicename=uniqid('NOK');
                       do {
                           ...
                           print "$White"; 
                           printf("%-6s$Green #%2s > ","rating","$xcount");
                    ...
                    if ($json != NULL)
                    {
                        ...
                        if (strstr($postmessage,"Post rating")) {
                                $msgs = "earnings saved\n";
                                $timetosleep=0;                  }
                            ...
                            if (strstr($postmessage,'Congratulations')) {
                               $msgs = "earnings saved\n";
                               $timetosleep=0;                          } 
                           if ($postmessage=="You already have rated this post.")
                           {
                            $msgs = "post already rated\n";
                            $timetosleep=0;   } 
